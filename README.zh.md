...
- 60% 的开源维护者经历过倦怠（Tidelift 2024）。库在无声无息中被遗弃。
- `npm audit` 只检查安全漏洞，不会告诉你一个库是否正在走向死亡。
- 当你注意到的时候，迁移成本已经很高了。

Drift 回答一个问题：**"这个库在6个月后还会被维护吗？"**

**适用场景：**

- **新项目启动** — 检查你即将依赖的库是否健康
- **定期维护** — 定期扫描项目，及早发现衰退的依赖
- **技术债务评审** — 使用 `--format json` 生成报告，为迁移方案提供依据
- **CI 流水线** — 检测到 Risk 或 Dead 依赖时自动构建失败
